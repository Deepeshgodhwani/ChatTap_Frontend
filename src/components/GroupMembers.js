...
function GroupMembers(props) {
  ...
  //searching members to add in a group
  ...
  //collecting members to add in a group //
  ...
  // removing member from selected members
  const removeUser = (user) => {
    setselectedUsers(
      selectedUsers.filter((members) => {
        return members.user._id !== user._id;
      })
    );

    setselectedUsersId(
      selectedUsersId.filter((members) => {
        return members.user !== user._id;
      })
    );
  };

  //adding collected members in a group
    ...
    <div className="">
      <div className="flex pt-4 px-8 2xl:px-10   justify-between">
        <div className="flex   space-x-2">
          ...
          ></img>
          <p className="text-[rgb(167,169,171)] text-sm font-semibold">
        ...
        >
          ...
          >
            <div className="flex bg-[rgb(36,36,36)] py-2  text-xl px-4 items-center space-x-6 ">
              <i
                onClick={closeTab}
                className=" cursor-pointer text-[rgb(111,111,111)] fa-solid fa-xmark"
              ></i>
              <p className="font-semibold">Add members</p>
            </div>
            <ModalBody padding={"0"}>
              <FormControl mt={4}>
                <div className="flex px-6   overflow-y-scroll styleScroll  max-h-16 flex-wrap gap-y-1 gap-x-1 my-2">
                  {selectedUsers.map((members) => {
                    return members.user._id !== logUser._id ? (
                      <div
                ...
                <input
                  onChange={onChange}
                  value={search}
                  className={`mx-6 border-[rgb(156,150,150)] px-4 outline-none 
              ...
              <div className="flex mt-4 flex-col overflow-y-scroll h-72 styleScroll my-2">
                {users.map((user) => {
                  return (
                    <div
                      onClick={(e) => {
                      ...
                      className="space-x-2 px-8 py-1 hover:bg-[rgb(58,58,58)]   cursor-pointer items-center flex "
                      key={user._id}
                    >
                      <img
                        alt=""
              ...
              {selectedUsers.length >= 1 && (
                <span
                  onClick={addUsers}
                  className="py-2 px-3 cursor-pointer absolute bottom-5 right-6 rounded-full bg-[rgb(38,141,97)]"
                >
        ...
        >
          ...
          <div className="flex">
            <p className=" text-base font-semibold">
              {logUser._id === Profile.admin._id
                ? "You"
                : Profile.admin.name.length > 15
                ? Profile.admin.name.slice(0, 15) + ".."
                : Profile.admin.name}
            </p>
            <p
              className="text-xs absolute right-2  py-[5px] font-bold px-2 rounded-md  text-white
                     bg-[rgb(53,55,59)] "
            >
              Group Admin
            </p>
        ...
        {renderMembers.map((members) => {
          return !members.isRemoved &&
            ...
            >
              ...
              {logUser._id === Profile.admin._id && (
                <div className=" cursor-pointer group-hover:flex hidden right-4 absolute">
                  <Popover>
                    <PopoverTrigger>
                    ...
                    >
                      <p
                        className="text-sm border-[rgb(75,75,75)] hover:bg-[rgb(58,58,58)]  border-b-[1px] py-1 "
                        onClick={() => {
                      ...
                      <p
                        ...
                        className="text-sm hover:bg-[rgb(58,58,58)] py-1"
                      >
                        View profile
                      </p>
                    </PopoverContent>
